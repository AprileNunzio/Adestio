        ...
        if (Array.isArray(peers)) {
            ...
            for (const peer of peers) {
                if (peer && typeof peer.ip === 'string' && peer.ip !== '127.0.0.1') {
                    // Validazione IP basic
                    const ipParts = peer.ip.split('.');
                    if (ipParts.length === 4) {
                        // Pass latency 0 so it doesn't immediately mark it as fully verified, 
                        // but antiEntropySweep will try to connect to it.
                        addNodeToMemory(peer.name || 'Adestio Node PEX', peer.ip, peer.port || 34567, 0, peer.protocolVersion || 0, peer.nodeId);
                    }
                }
            }

            // Ritorniamo i nostri nodi attivi
