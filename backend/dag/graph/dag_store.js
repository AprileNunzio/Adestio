...
function getBlocksSince(knownBlockIds) {
    ...
    for (let i = 0; i < toSend.length; i += CHUNK) {
        ...
        const rows = db.query(
            `SELECT block_id, parent_ids, event_type, table_name, record_id, payload, node_id, created_at, payload_version FROM event_log WHERE block_id IN (${chunk.map(() => '?').join(',')}) ORDER BY created_at ASC`,
            chunk
        );
...
function getPendingBlocks() {
    const db = getLedgerDB();
    const res = db.query('SELECT * FROM event_log WHERE is_applied = 0 ORDER BY created_at ASC');
    return res ? res.map(deserializeRow) : [];
}

module.exports = { getLedgerDB, getBlockById, getAllBlocks, getBlocksSince, isBlockKnown, getPendingBlocks };
