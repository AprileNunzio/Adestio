...
function reapplyToTables(block) {
    try {
        ...
        if (!SYNC_TABLES.includes(block.table_name)) return false;
        const dataDb = require('../../db').getDB(getDomainForTable(block.table_name));
        const adapted = adaptPayload(block.table_name, block.payload_version || 1, norm);
        return _applyToTable(dataDb, block.event_type, block.table_name, block.record_id, adapted, block.created_at);
    } catch (e) {
