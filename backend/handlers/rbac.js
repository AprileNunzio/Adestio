                    ...
                    try {
                        ...
                        if (m.permissions && Array.isArray(m.permissions)) {
                            for (const p of m.permissions) {
                                        const permId = `${d.name}:${p.id}`;
                                        const chk = db.exec(`SELECT * FROM permissions WHERE id = '${permId}'`);
                                        if (chk.length === 0) {
                                            db.run("INSERT INTO permissions (id, name, description, last_modified, is_deleted) VALUES (?, ?, ?, ?, 0)", 
                                                [permId, p.label || p.id, `Permesso ${p.id} per app ${d.name}`, ts]);

                                            if (p.default_groups && Array.isArray(p.default_groups)) {
                                                for (const gName of p.default_groups) {
                                                    const groupChk = db.exec(`SELECT id FROM groups WHERE name = '${gName}'`);
                                                    if (groupChk.length > 0 && groupChk[0].values.length > 0) {
                                                        const gId = groupChk[0].values[0][0];
                                                        db.run("INSERT INTO group_permissions (group_id, permission_id, last_modified, is_deleted) VALUES (?, ?, ?, 0)", [gId, permId, ts]);
                                                    }
                                                }
                                            }
                                        }
                                ...
                                try {
                                    ...
                                    if (sm.permissions && Array.isArray(sm.permissions)) {
                                        for (const p of sm.permissions) {
                                            const permId = `${d.name}:${sd.name}:${p.id}`;
                                            const chk = db.exec(`SELECT * FROM permissions WHERE id = '${permId}'`);
                                            if (chk.length === 0) {
                                                db.run("INSERT INTO permissions (id, name, description, last_modified, is_deleted) VALUES (?, ?, ?, ?, 0)", 
                                                    [permId, p.label || p.id, `Permesso ${p.id} per subapp ${d.name}/${sd.name}`, ts]);
...
function getGroupPermissions(event, groupId) {
    try {
        const db = getDB();
        if (!db) return [];
        const res = db.exec(`SELECT permission_id FROM group_permissions WHERE group_id = '${groupId}' AND is_deleted = 0`);
        return rowsToObjects(res).map(r => r.permission_id);
    } catch (e) { return []; }
}
...
function getUserPermissions(event, userId) {
    try {
        const db = getDB();
        if (!db) return [];
        const res = db.exec(`SELECT permission_id FROM user_permissions WHERE user_id = '${userId}' AND is_deleted = 0`);
        return rowsToObjects(res).map(r => r.permission_id);
    } catch (e) { return []; }
}
...
function getEffectiveUserPermissions(event, userId) {
    try {
        ...
        try {
            const superCheck = db.exec(`SELECT is_superadmin FROM users WHERE id = '${userId}'`);
            if (superCheck.length > 0 && superCheck[0].values.length > 0) {
                if (superCheck[0].values[0][0] === 1) {
                    return ['*'];
                }
            }
        } catch (colErr) {
        ...
        try {
            const superGroupCheck = db.exec(`
                SELECT 1 FROM groups g
                JOIN user_groups ug ON ug.group_id = g.id
                WHERE ug.user_id = '${userId}' AND ug.is_deleted = 0 AND g.is_superadmin = 1
                LIMIT 1
            `);
            if (superGroupCheck.length > 0 && superGroupCheck[0].values.length > 0) {
                return ['*'];
            }
        } catch (colErr) {
        ...
        const query = `
            SELECT permission_id FROM user_permissions WHERE user_id = '${userId}' AND is_deleted = 0
            UNION
            SELECT permission_id FROM group_permissions WHERE group_id IN (
                SELECT group_id FROM user_groups WHERE user_id = '${userId}' AND is_deleted = 0
            ) AND is_deleted = 0
        `;
...
function setGroupPermission(event, groupId, permissionId, value) {
    try {
        ...
        if (!db) return false;
        const ts = getTimestamp();
        const chk = db.exec(`SELECT * FROM group_permissions WHERE group_id = '${groupId}' AND permission_id = '${permissionId}'`);
...
function setUserPermission(event, userId, permissionId, value) {
    try {
        ...
        if (!db) return false;
        const ts = getTimestamp();
        const chk = db.exec(`SELECT * FROM user_permissions WHERE user_id = '${userId}' AND permission_id = '${permissionId}'`);
...
function getGroupUsers(event, groupId) {
    try {
        const db = getDB();
        if (!db) return [];
        const res = db.exec(`SELECT user_id FROM user_groups WHERE group_id = '${groupId}' AND is_deleted = 0`);
        return rowsToObjects(res).map(r => r.user_id);
    } catch (e) { return []; }
}
...
function updateGroupUsers(event, groupId, userIds) {
    try {
        ...
        for (const uid of userIds) {
            const chk = db.exec(`SELECT * FROM user_groups WHERE group_id = '${groupId}' AND user_id = '${uid}'`);
            if (chk.length > 0 && chk[0].values.length > 0) {
