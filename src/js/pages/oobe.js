...
function promptNetworkCode(el, node) {
    try {
        ...
        titleDesc.innerHTML = `Connessione a: <b>${node.name}</b><br><span style="font-family: monospace; font-size: 0.85rem; opacity: 0.8;">IPv4: ${node.host}</span>`;
        input.value = '';
                    ...
                    card.innerHTML = `
                        ...
                        <div style="flex: 1;">
                            ...
                            <div style="display: flex; gap: 1rem; font-size: 0.85rem; color: var(--md-on-surface-variant); font-family: monospace;">
                                <span><span class="material-symbols-rounded" style="font-size: 1rem; vertical-align: bottom;">lan</span> ${node.host}</span>
        ...
        try {
            ...
            if (unlockBtn) {
                unlockBtn.addEventListener('click', () => {
                    try {
                        // Simula l'apertura del prompt per il codice della blockchain
            ...
            if (importBtn) {
                importBtn.addEventListener('click', () => {
                    try {
                        ...
                        // Qui andrebbe integrata una chiamata a window.electronAPI.importBackup()
                    } catch (e) {}
                });
            }
